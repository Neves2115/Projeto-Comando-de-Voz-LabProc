...
## Rodada de correções v0.6.0 (bugs 16–20)

### 16. Erro espúrio "o echo nunca subiu" no monitoramento

O terminal enchia de erro **enquanto a distância aparecia certa no LCD**, e a
leitura avulsa não reclamava nunca.

A diferença estava no ritmo, não no código: `read_cm` dispara 3 amostras
seguidas, sem intervalo. O datasheet do HC-SR04 exige ~60 ms entre disparos —
antes disso o sensor ainda processa o eco anterior e simplesmente ignora o
trigger, então o ECHO nunca sobe. No modo avulso havia segundos entre chamadas e
o problema não aparecia; no contínuo, as leituras vinham uma atrás da outra.

Havia um segundo problema somado: **uma** amostra perdida já marcava o sensor
como defeituoso e disparava `logger.error`. Perder uma amostra é normal — eco em
superfície macia, quina, ruído.

**Correção:** `_wait_for_interval()` garante os 60 ms entre disparos; a rotina
de leitura é a mesma nos dois modos; falha isolada vira `debug`; só
`FAILURES_BEFORE_FAULT` (3) leituras seguidas sem nenhuma amostra válida
registram erro.

### 17. Brilho apagava a luz em qualquer porcentagem

`brightness()` escalava a cor atual. Com o LED apagado, a cor atual é preto — e
preto vezes qualquer fator continua preto. Qualquer porcentagem "apagava".

**Correção:** o brilho virou um fator próprio (`_brightness`), aplicado sobre o
matiz. Com a luz apagada, acende branco no brilho pedido. O fator persiste ao
trocar de cor e volta a 100% no `off()` — senão "desligar" e "ligar" acenderia
fraco sem motivo aparente.

### 18. Desenhos rotacionados

**Correção:** `rotate_ccw()` aplicado centralmente em `LedMatrix.show_rows()`,
com `[matrix] quarter_turns` configurável. Nenhum padrão em `ICONS` foi tocado,
então desenhos futuros já nascem orientados.

A primeira implementação estava errada — girava pela anti-diagonal em vez do
anti-horário. Passou despercebido porque a seta usada no teste visual é
simétrica; o teste com um único ponto no canto revelou. Fórmula correta:
`novo[i][j] = antigo[j][N-1-i]`.

### 19. "Parabéns pra você" truncada

Só as duas primeiras frases estavam na tabela. Adicionadas a terceira (que sobe
uma oitava) e a quarta. As demais melodias não foram tocadas.

### 20. Piscar ignorando a cor — **não reproduzido em software**

O roteador extrai `cor=laranja` corretamente e o `MockRgbLed` aplica exatamente
`(1.0, 0.45, 0.0)`. A lógica está certa, então o sintoma vem do hardware.

Ainda assim, a prioridade de cor foi tornada explícita (cor pedida > cor acesa >
branco) e coberta por testes. Para identificar a causa real na bancada, criei
`voz rgb-test`, que separa as duas hipóteses: polaridade invertida
(`rgb_active_high`) e PWM que não gradua.
