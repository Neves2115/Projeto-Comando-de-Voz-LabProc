...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser(
        "rgb-test",
        help="testa o LED RGB canal a canal e verifica se o PWM gradua",
    )
...
def _cmd_rgb_test(config: AppConfig) -> int:
    """Diagnostica o LED RGB: polaridade, canais e graduacao de brilho.

    Existe porque dois sintomas comuns tem a mesma raiz e sao dificeis de
    distinguir no olho: `rgb_active_high` invertido e PWM que nao gradua.
    """
    with build_hardware(config) as hardware:
        leds = hardware.leds
        pinos = config.pins
        print(
            f"\nLED RGB: R=GPIO{pinos.rgb_red} G=GPIO{pinos.rgb_green} "
            f"B=GPIO{pinos.rgb_blue}  |  active_high={pinos.rgb_active_high}\n"
        )

        print("1/3 Canais isolados — cada um deve acender SOZINHO na cor certa.")
        for nome in ("vermelho", "verde", "azul"):
            print(f"     {nome} ...")
            leds.set_named(nome)
            time.sleep(1.5)
        leds.off()

        print("\n2/3 Graduação — o branco deve escurecer aos poucos, não piscar.")
        leds.set_named("branco")
        for pct in (100, 75, 50, 25, 10):
            print(f"     {pct}%")
            leds.brightness(pct / 100.0)
            time.sleep(1.2)
        leds.off()

        print("\n3/3 Cor misturada — deve aparecer LARANJA, não branco.")
        leds.set_named("laranja")
        time.sleep(2.5)
        leds.off()

    print(
        "\nComo interpretar:\n"
        "  Cores trocadas (pede vermelho, acende ciano)\n"
        "      -> polaridade invertida. Em config.toml:\n"
        "             [pins]\n             rgb_active_high = "
        f"{str(not config.pins.rgb_active_high).lower()}\n"
        "  Etapa 2 não escurece (só liga/desliga), ou a etapa 3 sai branca\n"
        "      -> o PWM não está graduando. Use PWM com timing de hardware:\n"
        "             sudo apt install -y pigpio python3-pigpio\n"
        "             sudo systemctl enable --now pigpiod\n"
        "             voz run --pin-factory pigpio\n"
        "  Um canal não acende\n"
        "      -> fio ou pino daquele canal.\n"
    )
    return 0
