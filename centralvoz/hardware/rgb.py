...
class RgbLed(Peripheral):
    ...
    def set_named(self, name: str) -> bool:
        """Acende uma cor pelo nome. False se o nome nao existe."""
        cor = resolve_color(name)
        if cor is None:
            return False
        self.set_color(cor, name.lower())
        return True
    ...
    def brightness(self, factor: float) -> None:
        """Escala a cor atual (0..1) sem mudar o matiz."""
        factor = max(0.0, min(1.0, factor))
        base = resolve_color(self._color_name) or self._color
        self._set_color_raw(tuple(canal * factor for canal in base), self._color_name)
    ...
    def blink(
        ...
        alvo = resolve_color(color) or (
            self._color if self.is_on else COLORS["branco"]
        )
        nome = color or self._color_name
