...
def test_leitura_com_sensor_mudo_devolve_none_rapido() -> None:
    ...
    # Uma leitura falha nao marca o sensor como defeituoso: pode ser eco perdido.
    assert sensor.faulty is False


def test_sensor_so_e_dado_como_mudo_apos_falhas_seguidas() -> None:
    """Regressao: o modo continuo enchia o terminal de erro a cada amostra perdida.

    Uma amostra perdida e normal (eco em superficie macia, quina, ruido). So um
    conjunto de falhas seguidas significa problema de fiacao.
    """
    from centralvoz.hardware.distance import FAILURES_BEFORE_FAULT, MockDistanceSensor

    sensor = MockDistanceSensor(14, 15)
    sensor.simulate_timeout = True

    for _ in range(FAILURES_BEFORE_FAULT - 1):
        assert sensor.read_cm(samples=1) is None
        assert sensor.faulty is False
...
def test_amostra_perdida_no_meio_nao_invalida_a_leitura() -> None:
    """A distancia continua correta e nenhum erro e registrado."""
    from centralvoz.hardware.distance import MockDistanceSensor

    sensor = MockDistanceSensor(14, 15, value_cm=30.0)
    chamadas = {"n": 0}
    original = sensor._read_once

    def instavel():
        chamadas["n"] += 1
        if chamadas["n"] == 2:  # a segunda amostra falha
            raise RuntimeError("eco perdido")
        return original()

    sensor._read_once = instavel
    valor = sensor.read_cm(samples=3)
...
def test_leituras_seguidas_respeitam_o_intervalo_minimo() -> None:
    """O HC-SR04 ignora disparos muito proximos e o ECHO nunca sobe.

    Era a causa de "o echo nunca subiu" aparecer so no modo continuo.
    """
    import time

    from centralvoz.hardware.distance import MIN_INTERVAL_S, MockDistanceSensor

    sensor = MockDistanceSensor(14, 15)
    inicio = time.monotonic()
    sensor.read_cm(samples=3)
    decorrido = time.monotonic() - inicio

    assert decorrido >= MIN_INTERVAL_S * 2
...
def test_brilho_com_led_apagado_acende_branco() -> None:
    """Regressao: qualquer porcentagem apagava a luz.

    Escalar a cor atual funcionava so com o LED aceso -- preto vezes qualquer
    fator continua preto.
    """
    from centralvoz.hardware.rgb import MockRgbLed

    led = MockRgbLed(5, 6, 13)
    assert led.is_on is False

    led.brightness(0.5)
    assert led.is_on is True
    assert led.color == pytest.approx((0.5, 0.5, 0.5))


@pytest.mark.parametrize(
    ("pct", "esperado"), [(0, 0.0), (25, 0.25), (50, 0.5), (100, 1.0)]
)
def test_brilho_e_proporcional(pct: int, esperado: float) -> None:
    from centralvoz.hardware.rgb import MockRgbLed

    led = MockRgbLed(5, 6, 13)
    led.set_named("vermelho")
    led.brightness(pct / 100.0)
    assert led.color[0] == pytest.approx(esperado)


def test_brilho_persiste_ao_trocar_de_cor() -> None:
    from centralvoz.hardware.rgb import MockRgbLed
    ...
    assert led.color == pytest.approx((0.0, 0.0, 0.3))
    assert led.color_name == "azul"


def test_desligar_restaura_o_brilho_maximo() -> None:
    """Senao 'desligar' e depois 'ligar' acenderia fraco sem motivo aparente."""
    from centralvoz.hardware.rgb import MockRgbLed
    ...
    assert led.color == pytest.approx((0.0, 1.0, 0.0))


# --------------------------------------------------------------------------- #
# Piscar com cor (Bug 5)
# --------------------------------------------------------------------------- #


def test_piscar_usa_a_cor_pedida() -> None:
    from centralvoz.hardware.rgb import COLORS, MockRgbLed

    led = MockRgbLed(5, 6, 13)
    aplicadas = []
    original = led._apply
    led._apply = lambda cor: (aplicadas.append(cor), original(cor))

    led.blink(times=2, on_time=0.01, off_time=0.01, color="laranja")

    acesas = {c for c in aplicadas if any(c)}
    assert acesas == {COLORS["laranja"]}


def test_piscar_sem_cor_usa_branco_quando_apagado() -> None:
    from centralvoz.hardware.rgb import COLORS, MockRgbLed
    ...
    led.blink(times=1, on_time=0.01, off_time=0.01)
    assert COLORS["branco"] in aplicadas


def test_piscar_sem_cor_mantem_a_cor_acesa() -> None:
    from centralvoz.hardware.rgb import COLORS, MockRgbLed
    ...
    led.blink(times=1, on_time=0.01, off_time=0.01)
    assert COLORS["branco"] not in aplicadas
    assert COLORS["verde"] in aplicadas


# --------------------------------------------------------------------------- #
# Rotacao da matriz (Bug 3)
# --------------------------------------------------------------------------- #


def test_rotacao_e_anti_horaria() -> None:
    from centralvoz.hardware.matrix import pattern_to_rows, rotate_ccw

    # Uma marca isolada no canto superior esquerdo.
    padrao = ["#......."] + ["........"] * 7
    girado = rotate_ccw(pattern_to_rows(padrao))

    # No anti-horario, o canto superior direito vira o superior esquerdo.
    # Logo o superior esquerdo desce para o inferior esquerdo.
    assert girado[7] == 0b10000000


def test_quatro_giros_voltam_ao_original() -> None:
    from centralvoz.hardware.matrix import ICONS, pattern_to_rows, rotate

    for nome, padrao in ICONS.items():
        linhas = pattern_to_rows(padrao)
        assert rotate(linhas, 4) == linhas, nome


def test_rotacao_aplicada_a_todos_os_icones() -> None:
    """A rotacao e central: nenhum desenho pode escapar."""
    from centralvoz.hardware.matrix import ICONS, MockMatrix, pattern_to_rows, rotate

    sem_giro = MockMatrix(quarter_turns=0)
    com_giro = MockMatrix(quarter_turns=1)

    for nome in ICONS:
        sem_giro.show_icon(nome)
        com_giro.show_icon(nome)
        esperado = rotate(pattern_to_rows(ICONS[nome]), 1)
        assert com_giro._rows == esperado, nome


def test_rotacao_configuravel(tmp_path) -> None:
    from centralvoz.config import AppConfig

    arquivo = tmp_path / "config.toml"
    arquivo.write_text("[matrix]\nquarter_turns = 3\n")
    assert AppConfig.load(arquivo).matrix.quarter_turns == 3


# --------------------------------------------------------------------------- #
# Melodia completa (Bug 4)
# --------------------------------------------------------------------------- #


def test_parabens_tem_as_quatro_frases() -> None:
    """Regressao: a musica parava depois da segunda frase."""
    from centralvoz.hardware.buzzer import MELODIES

    melodia = MELODIES["parabens"]
    assert len(melodia) >= 25

    duracao = sum(d for _, d in melodia)
    assert 8.0 <= duracao <= 14.0

    # A terceira frase sobe uma oitava; sem ela a musica esta truncada.
    assert any(nota == "do5" for nota, _ in melodia)


def test_outras_melodias_nao_mudaram() -> None:
    from centralvoz.hardware.buzzer import MELODIES

    assert len(MELODIES["ok"]) == 3
    assert len(MELODIES["erro"]) == 2
    assert len(MELODIES["alerta"]) == 3
